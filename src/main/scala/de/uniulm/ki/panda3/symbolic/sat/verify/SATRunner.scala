...
case class SATRunner(domain: Domain, initialPlan: Plan, satSolver: Solvertype, timeCapsule: TimeCapsule, informationCapsule: InformationCapsule) {
  ...
  def runWithTimeLimit(timelimit: Option[Long], planLength: Int, offsetToK: Int, includeGoal: Boolean = true, defineK: Option[Int] = None): (Boolean, Boolean) = {
    val runner = new Runnable {
      var result: Option[Boolean] = None

      override def run(): Unit = {
        result = Some(SATRunner.this.run(planLength: Int, offsetToK, includeGoal, defineK))
      }
    }
    // start thread
  ...
  def run(planLength: Int, offSetToK: Int, includeGoal: Boolean = true, defineK: Option[Int] = None): (Boolean) = try {
    ...
    val writer = new BufferedWriter(new FileWriter(new File(fileDir + "__cnfString")))
    ...
    try {
      satSolver match {
        case MINISAT()       =>
          ...
          ("minisat " + fileDir + "__cnfString " + fileDir + "__res.txt") !
        case CRYPTOMINISAT() =>
          println("Starting cryptominisat5")
          ("cryptominisat5 --verb 0 " + fileDir + "__cnfString") #> new File(fileDir + "__res.txt") !
      }
    } catch {
    ...
    // postprocessing
    val solverOutput = Source.fromFile(fileDir + "__res.txt").mkString
    ...
    // delete files
    ("rm " + fileDir + "__cnfString " + fileDir + "__res.txt") !
