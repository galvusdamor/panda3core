...
case class VerifyRunner(domFile: String, probFile: String, configNumber: Int, parserType: ParserType, satsolver: Solvertype) {
  ...
  lazy val (solutionPlan, domain, initialPlan, preprocessTime) = {
    ...
    val (searchConfig, usePlanningGraph) = configNumber match {
      ...
      case 1          => (PlanBasedSearch(None, None, AStarDepthType, Some(TDGMinimumModificationWithCycleDetection), Nil,LCFR), true)
      case 2          => (PlanBasedSearch(None, None, DijkstraType, None, Nil,LCFR), true)
      case 3          => (PlanBasedSearch(None, None, AStarDepthType, Some(TDGMinimumAction), Nil,LCFR), true)
      case 4          => (PlanBasedSearch(None, None, AStarDepthType, Some(TDGMinimumModificationWithCycleDetection), Nil,LCFR), false)
