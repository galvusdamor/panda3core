...
case class SOGPOCLDeleteEncoding(timeCapsule: TimeCapsule, domain: Domain, initialPlan: Plan,
  ...
  def causalThreatsFormula(supporterLiterals: Seq[(Seq[Int], Seq[Int], Predicate)]) : Seq[Clause] = {
    ...
    // no causal threats
    val potentialThreader: Seq[(Seq[Int], Seq[Int], Seq[Int], Predicate)] = supporterLiterals flatMap { case (p1, p2, prec) =>
    ...
    val noCausalThreat = potentialThreader map { case (p1, p2, pt, prec) =>
      impliesRightOr(supporter(p1, p2, prec) :: deletes(pt, prec) :: Nil, before(pt, p1) :: before(p2, pt) :: Nil)
    }
