...
/**
  ...
  */
case class SOGPOCLForbidEffectEncoding(timeCapsule: TimeCapsule, domain: Domain, initialPlan: Plan, intProblem: IntProblem,
  ...
  override val enforceTotalOrder: Boolean = true
  ...
  def causalThreatsFormula(supporterLiterals: Seq[(Seq[Int], Seq[Int], Predicate)]): Seq[Clause] = {
    ...
    val noCausalThreat: Seq[Clause] = potentialThreader map { case (p1, p2, t, pt, prec, supporterLiteral) =>
      impliesRightNotAll(supporterLiteral :: pathAction(pt.length, pt, t) :: before(p1, pt) :: before(pt, p2) :: Nil, Nil)
    }
