...
import de.uniulm.ki.panda3.symbolic.domain.{DecompositionMethod, GeneralTask, ReducedTask, Task}
...
case class SymbolicPlan(planStepsAndRemovedPlanSteps: Seq[PlanStep], causalLinksAndRemovedCausalLinks: Seq[CausalLink], orderingConstraints: TaskOrdering, parameterVariableConstraints: CSP,
                        ...
                        planStepDecomposedByMethod: Map[PlanStep, DecompositionMethod], planStepParentInDecompositionTree: Map[PlanStep, (PlanStep, PlanStep)]) extends Plan with HashMemo {
  ...
  override lazy val abstractPlanSteps: Seq[AbstractPlanStep] = planSteps filter {
  ...
  override def modify(modification: Modification): SymbolicPlan = {
    ...
    SymbolicPlan(newPlanStepsIncludingRemovedOnes, newCausalLinks, newOrderingConstraints, newVariableConstraints, init, goal, isModificationAllowed, isFlawAllowed,
  ...
  override def newInstance(firstFreePlanStepID: Int, firstFreeVariableID: Int, partialSubstitution: Substitution[Variable], parentPlanStep: PlanStep):
  (Plan, Substitution[Variable], Map[PlanStep, PlanStep]) = {
    ...
    val planStepMapping = planSteps zip (firstFreePlanStepID until firstFreePlanStepID + planSteps.size) map { case (ps, id) =>
    ...
    (SymbolicPlan(newPlanSteps, newCausalLinks, newOrderingConstraints, newVariableConstraints, newInit, newGoal, isModificationAllowed, isFlawAllowed, Map(), Map()), sub,
  ...
  override def update(domainUpdate: DomainUpdate): SymbolicPlan = domainUpdate match {
    ...
    case AddLiteralsToInitAndGoal(literalsInit, literalsGoal, constraints) => {
      ...
      SymbolicPlan(planStepsAndRemovedPlanSteps map { _ update exchangeInit update exchangeGoal }, causalLinksAndRemovedCausalLinks map { _ update exchangeInit update exchangeGoal },
    ...
    case _                                                                 => SymbolicPlan(planStepsAndRemovedPlanSteps map { _.update(domainUpdate) },
                                                                                           causalLinksAndRemovedCausalLinks map { _.update(domainUpdate) },
                                                                                           ...
                                                                                           planStepDecomposedByMethod map { case (ps, method) => (ps update domainUpdate, method update
                                                                                             domainUpdate) },
                                                                                           planStepParentInDecompositionTree map { case (ps, (parent, inPlan)) =>
