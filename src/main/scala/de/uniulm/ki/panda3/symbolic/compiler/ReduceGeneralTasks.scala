...
object ReduceGeneralTasks  extends DomainTransformer[Unit] {
  ...
  override def transform(inDomain: Domain, inPlan: Plan, info: Unit): (Domain, Plan) = {
    ...
    var domain = inDomain update ExchangeTask(replacements)
    var plan = inPlan update ExchangeTask(replacements)

    generalTasks = filterGeneralTasks(domain.tasks ++ artificialTasks(domain, plan))

    while (generalTasks.nonEmpty) {
      val gt  = generalTasks.head
      val simp = simplification(gt)

      if (simp.isRight) {
        val (d, p, newTasks) = simp.right.get(domain, plan)
        val newGeneralTasks = filterGeneralTasks(newTasks)
        domain = d
        plan = p
        generalTasks = generalTasks.tail ++ newGeneralTasks
      } else {
        domain = domain update ExchangeTask(Map(gt -> simp.left.get))
        plan = plan update ExchangeTask(Map(gt -> simp.left.get))
        generalTasks = generalTasks.tail
      }
    }
    (domain update ReduceTasks(), plan update ReduceTasks())
  }
