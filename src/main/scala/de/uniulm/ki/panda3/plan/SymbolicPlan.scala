...
case class SymbolicPlan(domain: Domain, planSteps: Seq[PlanStep], causalLinks: Seq[CausalLink], orderingConstraints: TaskOrdering, variableConstraints: CSP, init: PlanStep,
  ...
  override lazy val unboundVariables: Seq[UnboundVariable] = (variableConstraints.variables collect {
    case v if variableConstraints.getRepresentative(v).isLeft => variableConstraints.getRepresentative(v) match {
      case Left(v) => v
    }
  } map {
  ...
  override def newInstance(firstFreePlanStepID: Int, firstFreeVariableID: Int): (Plan, Substitution) = {
    val oldPlanVariables = variableConstraints.variables.toSeq

    val newVariables = oldPlanVariables zip (firstFreeVariableID until firstFreeVariableID + oldPlanVariables.size) map {case (v, id) => Variable(id, v.name, v.sort)}
    val sub = Substitution(oldPlanVariables, newVariables)

    val newPlanSteps = planSteps zip (firstFreePlanStepID until firstFreePlanStepID + planSteps.size) map {
      case (ps, id) => PlanStep(id, ps.schema, ps.arguments map sub)
    }
